...
interface ActivationInfo {
  ...
  steps?: string[]; // Steps for detailed view
}
...
// All Activations Data - Combined and enhanced with IDs and default details
const allActivationsData: ActivationInfo[] = [
  ...
  },
  {
    ...
    image: "https://storage.obdeleven.com/parse/98e61f5539a545cfede9847ac8f79d3a_oca_picture.jpg",
    year: "2020-2024",
...
const HomePage: React.FC = () => {
  ...
  const [currentDetailedActivation, setCurrentDetailedActivation] = useState<ActivationInfo | null>(null); // Use ActivationInfo type
...
useEffect(() => {
  if (selectedCar) {
    // فقط قم بضبط رسالة الاتصال إذا كانت فارغة أو تحتوي فقط على نص السيارة السابق
    // هذا يمنع استبدال نص المستخدم إذا كان قد بدأ بالكتابة بالفعل
    ...
    // إذا لم يتم اختيار سيارة، قم بمسح الرسالة إذا كانت تحتوي على نص السيارة الافتراضي
    if (contactMessage.startsWith('أود الاستفسار عن تفعيل لسيارة:')) {
      setContactMessage('');
    }
  }
}, [selectedCar, contactMessage]); // يجب أن تعتمد على contactMessage أيضاً لتجنب الحلقات اللانهائية.

  // Function to handle form submission to the Webhook
const handleSubmit = async (e: React.FormEvent) => {
  e.preventDefault(); // Prevent default form submission

  // رابط Webhook الأساسي بدون أي معلمات استعلام
  const BASE_WEBHOOK_URL = "https://trigger.macrodroid.com/9519b967-983a-4878-a3c7-7d9560ffa0f7/bmw";

  // تحضير البيانات لإرسالها كمعلمات استعلام
  // تأكد من أن مفاتيح هذه الكائنات (مثل 'name', 'email') تتطابق مع ما يتوقعه Macrodroid في إعدادات الـ Webhook.
  ...
  }).toString(); // تحويل الكائن إلى سلسلة معلمات استعلام (مثل key1=value1&key2=value2)

  // بناء الرابط النهائي مع معلمات الاستعلام
  const finalWebhookUrl = `${BASE_WEBHOOK_URL}?${queryParams}`;

  try {
    // إرسال طلب GET. لا توجد حاجة لـ headers أو body مع GET
    const response = await fetch(finalWebhookUrl, {
      method: 'GET', // تغيير طريقة الطلب إلى GET
    });
    ...
    if (response.ok) {
      ...
      // إذا لم تكن الاستجابة ناجحة (مثلاً 404 أو 500)، أظهر رسالة خطأ
    ...
    // التعامل مع أخطاء الشبكة (مثل عدم وجود اتصال بالإنترنت)
    alert('حدث خطأ في الاتصال. الرجاء التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.');
    console.error('Network error:', error);
  }
};
  // Filter activations based on selected car's year range
  const filteredActivations = selectedCar
    ...
    : []; // If no car is selected, show an empty array or allActivationsData if desired.
      ...
      <header className="bg-transparent backdrop-blur-md p-4 shadow-lg sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div className="w-12 h-12 bg-contain bg-center bg-no-repeat ml-3" // Changed mr-3 to ml-3 for RTL
